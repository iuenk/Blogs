function build-cpc-aib-image {
    ...
    begin {
        $PSDefaultParameterValues = @{
            "build-cpc-aib-image:armFile"=".\.json"
            "build-cpc-aib-image:baselineConfiguration"=".\.ps1"
          }
    }
    ...
    process {
        ...
        $saName = $customer + $solution + $environment
        $containerName = $customer + $solution + "repo"
        $blobName = "baseline-cpc-aib-image.ps1"
        $version = "latest"
        ...
        $Id = (Get-AzResourceGroup -Name $rgName).ResourceId
        $resourceId = $Id + "/providers/Microsoft.Compute/images/$runOutputName"
        ...
        ((Get-Content -path $armFile -Raw) -replace '<resourceId>',$resourceId) | Set-Content -Path $armFile
